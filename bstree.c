	...
	do
	 {
		...
		switch(c)
		 {
			case 1:printf("_Enter data to Insert:\n");
    			   scanf("%d",&d);
    			   insert(d);
       			   break;
			case 2:printf("_Enter data to Search:\n");
  		     	   scanf("%d",&d);
       			   search(root,d);//printf("%d",root->data);
     			   break;
			case 3:printf("_Enter data to Delete:\n");
       			   scanf("%d",&k);
       			   z=dele(k);
       			   if(z==0)
			...
			case 4:if(root==NULL)
					...
					{
						printf("_Inorder traversal Result: ");
   			       		inorder(root);
      			   		printf("\n");
					}
       			   break;       
			case 5:printf("____Exiting_!!____\n");
       			   exit(0);   
       			   printf("\n");                  
		 }
	}while(1);
...
void insert(int x)
 {
	...
	if(t1!=0)
   		printf("_Duplication Not Allowed_!\n");
...
//loop to find the node to be deleted
  ...
  //after the loop t1 points to node to be deleted
